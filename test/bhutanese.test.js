...
import { CalendarBhutanese } from '../src/index.js'

function toDate (jde) {
  return new julian.Calendar().fromJDE(jde).toDate()
}

describe('#CalendarBhutanese', function () {
  describe('construction', function () {
    let cal
    const exp = [17, 21, 1, false, 2, false]
    it('can construct via new', function () {
      cal = new CalendarBhutanese(17, 21, 1, false, 2, false)
      assert.ok(cal instanceof CalendarBhutanese)
      assert.deepStrictEqual(cal.get(), exp)
    })

    it('can construct with class instance', function () {
      const cal1 = new CalendarBhutanese(cal)
      assert.ok(cal1 instanceof CalendarBhutanese)
      assert.ok(cal1 !== cal)
      assert.deepStrictEqual(cal.get(), exp)
    })

    it('can construct with array', function () {
      const cal1 = new CalendarBhutanese(exp)
      assert.ok(cal1 !== cal)
      assert.deepStrictEqual(cal.get(), exp)
    })
  })
  ...
  describe('Gregorian', function () {
    const tests = [
      ...
      { d: [2003,  3,  4], tib: [17, 17, 1, false, 1, false] },
      ...
      { d: [2016,  9,  1], tib: [17, 30,  7, false, 30, false] },
      { d: [2016,  9,  2], tib: [17, 30,  7, true,   1, false] },
      { d: [2016, 10,  2], tib: [17, 30,  8, false,  1, false] },
      { d: [2019,  5,  4], tib: [17, 33,  3, false, 30, false] },
      { d: [2019,  5,  5], tib: [17, 33,  3, true,   1, false] },
      { d: [2019,  6,  4], tib: [17, 33,  4, false,  1, false] },
    ...
    describe('fromGregorian', function () {
      const cal = new CalendarBhutanese()
    ...
    describe('toGregorian', function () {
      tests.forEach(function (t) {
        ...
        it(t.d.join('-') + ' ' + [cycle, year, month, leapMonth, day, leapDay].join('-'), function () {
          const cal = new CalendarBhutanese(cycle, year, month, leapMonth, day, leapDay)
